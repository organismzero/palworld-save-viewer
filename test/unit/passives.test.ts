...
describe('the two distributions', () => {
  ...
  it('RANDOM_ADD is a separate roll over 0…3, averaging one per hatch', () => {
    // Separate is the load-bearing word. `Combi_PassiveInheritNum` and
    // `Combi_PassiveRandomAddNum` are two arrays in `BP_PalGameSetting` with
    // the same weights over different outcome ranges — 1–4 and 0–3 — which is
    // why there are two of them. The wiki instead describes the second roll as
    // the difference between two draws on the first distribution, which would
    // put this mean at 0.54 and roughly halve every junk estimate in the app.
    // Pinned here so that reading cannot be adopted by accident.
...
describe('pAtLeast', () => {
  ...
  it('lands a perfect four at one hatch in ten', () => {
    ...
    // nowhere to put a random one and the clean result is free. The community
    // reports ~10–15%, which is the one place this model is externally
    // calibrated, so it is pinned.
