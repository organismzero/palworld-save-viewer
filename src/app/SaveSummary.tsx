  ...
  return (
    <div className="mx-auto max-w-6xl p-8">
      ...
      {s.playerDetails < s.playersInLevel && (
        <Panel className="mb-10 px-5 py-4">
          <div className="flex flex-wrap items-baseline gap-x-3 gap-y-1">
            ...
            <span className="text-sm text-[var(--color-muted)]">
              ...
              {count(s.unattributedContainers)} containers stay unattributed and
              last-seen falls back to the server-uptime clock.
            </span>
          </div>
        </Panel>
      )}
        ...
        <Table
          ...
          rows={[...index.pals]
            ...
            .map((p) => [
              ...
              p.level,
              <span className="flex items-center gap-2">
                <IVBar hp={p.ivHp} attack={p.ivAttack} defense={p.ivDefense} />
      ...
      <section className="mb-10">
        ...
        <Panel className="px-5 py-4">
          {s.warnings.length === 0 ? (
            <p className="text-sm text-[oklch(0.78_0.16_150)]">
            ...
            <ul className="space-y-1 text-sm">
              {s.warnings.map((w) => (
                <li key={w.kind + w.detail}>
                  <span className="num text-[oklch(0.80_0.15_85)]">
                    {w.count}×
                  </span>{' '}
